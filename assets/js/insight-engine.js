...
export function generatePerformanceInsight({
  ...
  // If no data at all, return empty — caller should hide section
  ...
  // ---- Short, punchy 2-sentence pattern set ----
  const SHORT_PATTERNS = [
    // Accuracy + trend
    d => `${accuracyObservation(d.accuracy).split('.')[0]}. ${trendCommentary(d.trend).split('.')[0]}.`,

    // Practice mix focused
    d => {
      const mix = practiceBalance(d.rtp, d.mtp, d.chapter).split('.')[0];
      const acc = d.accuracy > 0 ? `Overall accuracy stands at ${d.accuracy}%.` : "";
      return [mix, acc].filter(Boolean).join(" ");
    },

    // Data summary + action
    d => {
      const total = d.rtp + d.mtp + d.chapter;
      const parts = [];
      if (d.chapter > 0) parts.push(`${d.chapter} chapter${d.chapter>1?'s':''}`);
      if (d.rtp > 0) parts.push(`${d.rtp} RTP`);
      if (d.mtp > 0) parts.push(`${d.mtp} MTP`);
      const summary = parts.length > 0
        ? `This period: ${parts.join(', ')} — ${total} attempt${total>1?'s':''} total.`
        : `${total} attempts logged this period.`;
      const insight = trendCommentary(d.trend).split('.')[0];
      return `${summary} ${insight}.`;
    },

    // Accuracy + subject specific
    d => {
      const acc = accuracyObservation(d.accuracy).split('.')[0];
      const sub = d.subject && d.subject !== "All Subjects" && d.subject !== "all"
        ? subjectInsight(d.subject).split('.')[0]
        : trendCommentary(d.trend).split('.')[0];
      return `${acc}. ${sub}.`;
    },

    // Greeting + key metric
    d => {
      const greeting = contextualGreeting().split('.')[0];
      const trend = trendCommentary(d.trend).split('.')[0];
      return `${greeting}. ${trend}.`;
    },

    // Mix + accuracy verdict
    d => {
      const rPct = d.totalAttempts > 0 ? Math.round((d.rtp/d.totalAttempts)*100) : 0;
      const mPct = d.totalAttempts > 0 ? Math.round((d.mtp/d.totalAttempts)*100) : 0;
      const cPct = d.totalAttempts > 0 ? Math.round((d.chapter/d.totalAttempts)*100) : 0;
      const mixLine = `Mix: ${cPct}% chapters, ${rPct}% RTP, ${mPct}% MTP.`;
      const accLine = d.accuracy > 0
        ? accuracyObservation(d.accuracy).split('.')[0] + "."
        : trendCommentary(d.trend).split('.')[0] + ".";
      return `${mixLine} ${accLine}`;
    },
  ];
  ...
  do {
    const pattern = pick(SHORT_PATTERNS);
