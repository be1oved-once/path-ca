...
function getAuthCache() {
  ...
  if (!raw) return undefined; // first load
...
window.__authUser = cachedUser;
window.__authReady = cachedUser !== undefined;
...
onAuthStateChanged(auth, async user => {
  ...
  // ⚡ SAVE CACHE FIRST (instant next page load)
  saveAuthCache(user);
...
(function () {
  ...
  document.addEventListener("click", e => {
    ...
    // ignore files that already have extension
    if (/\.(html|php|js|css|png|jpg|jpeg|svg|webp)$/i.test(href)) return;

    e.preventDefault();

    const url = href.replace(/\/$/, "") + ".html";
