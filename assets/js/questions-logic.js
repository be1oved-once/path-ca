...
async function handleAnswer(btn, uiIndex) {
  ...
  // ⚡ EARLY SAVE: If this is the last question in round 1, kick off
  // all 3 Firebase saves immediately — don't wait for "Next" click.
  // This prevents data loss if the user closes the page before pressing Next.
  if (round === 1 && !round1Completed && qIndex === activeQuestions.length - 1) {
    _triggerEarlySave();
  }
}

// ⚡ Fires all 3 saves the moment the last question is answered.
// finishRound() checks round1Completed so saves are never duplicated.
function _triggerEarlySave() {
  if (!currentUser || round1Completed) return;
  round1Completed = true;

  // 📸 Snapshot now — while all answers are fresh
  ...
  // 3️⃣ Corrections batch — single atomic roundtrip
  const correctionsPromise = (async () => {
    try {
      ...
      await batch.commit();
      console.log("✅ Early corrections saved:", wrongOnly.length);
    } catch (e) {
      console.error("❌ Early corrections save failed", e);
    }
  })();
...
if (!round1Completed) {
  ...
  // 🎯 UI — instant, no waiting for Firebase
  marksValue.textContent = marks.toFixed(2);
  marksBox.classList.remove("hidden");
  resultActions?.classList.remove("hidden");

  // ⚡ FIRE ALL 3 SAVES IN PARALLEL — no awaiting each other
  if (currentUser) {
    const wrongOnly = round1Snapshot.filter(q => !q.correct);
    const correctCount = round1Snapshot.filter(q => q.correct).length;
    const total = round1Snapshot.length;
    ...
    // 3️⃣ Corrections — writeBatch for atomic single-roundtrip write
    const correctionsPromise = (async () => {
      try {
        const colRef = collection(db, "users", currentUser.uid, "corrections");
        const old = await getDocs(colRef);
    ...
    // Run all 3 in parallel — fastest possible
    Promise.all([statsPromise, summaryPromise, correctionsPromise])
